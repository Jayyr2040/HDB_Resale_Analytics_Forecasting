...
if os.path.exists("scripts/coords_cache.json"):
    with open("scripts/coords_cache.json", "r") as f:
        cache = json.load(f)
else:
    cache = {}
...
if access_token_1:
    ...
    with open('scripts/coords_cache.json', "w") as f:
...
output_path = "scripts/enriched_hdb_resale.parquet"
...
def fetch_primary_schools_svy21(token):
   # Define a tracking path inside your working repository
    cache_file = "scripts/primary_schools_cache.json"
...
def fetch_MRT_LRT_svy21(token):
   # Define a tracking path inside your working repository
    cache_file = "scripts/mrt_lrt_cache.json"
...
def fetch_shopping_malls_svy21(token):
    ...
    # ─── STEP A: CHECK LOCAL STORAGE CACHE WITH 30-DAY TTL ───
    # 30 days in seconds = 30 * 24 * 60 * 60 = 2,592,000 seconds
    cache_file = "scripts/shopping_malls_cache.json"
